...
def create_cover():
    ...
    # ===== 4. 截图区域：放大，减少上下留白 =====
    # 修改点：截图尺寸加大，起始Y坐标上移
    ss_h = 560  # 原530 -> 560，更大更清晰
    ss_w = int(ss_h * 212 / 520)
    gap_ss = 14
    total_w = 4 * ss_w + 3 * gap_ss
    ss_x0 = (W - total_w) // 2
    ss_y0 = 155  # 原190 -> 155，利用上方空间
    cr = 10
    ...
    for i, pp in enumerate(PREVIEWS):
        ...
        ImageDraw.Draw(mask).rounded_rectangle([(0,0),(ss_w,ss_h)], r=cr, fill=255)
    ...
    # ===== 5. 底部信息：极度精简 =====
    # 修改点：删除GitHub链接和分割线，只保留版本号，字号缩小
